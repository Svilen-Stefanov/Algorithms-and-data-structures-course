...
public class RingQueue {
  ...
  private void setRange(Interval newRange) {
    if(newRange.isEmpty()) {
      from = 0;
      to = 0;
    } else {
      this.from = newRange.getFrom();
      this.to = newRange.getTo();
    }
  }
  ...
  public void enqueue(int value) {
    setRange(dynArr.reportUsage(isEmpty() ? new EmptyInterval() : new NonEmptyInterval(from, to), getSize() + 1));
    if(!isEmpty())
  ...
  public int dequeue() {
    ...
    if(from == to) {
      setRange(dynArr.reportUsage(new EmptyInterval(), 0));
    } else {
      from = (from + 1)%dynArr.getInnerLength();
      setRange(dynArr.reportUsage(new NonEmptyInterval(from, to), 0));
    }
    size--;
    return value;
  }
