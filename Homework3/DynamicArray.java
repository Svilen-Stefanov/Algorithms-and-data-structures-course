...
//    System.out.println("Reallocation!");
    if(lengthNew == 0) {
      ...
      int sizeNew = lengthNew * growthFactor;
      int[] elementsNew = new int[sizeNew];
      for (int i = 0; i < usage.getSize(elements.length); i++)
        elementsNew[i] = elements[(i + usage.getFrom())%elements.length];
  ...
  public Interval reportUsage(Interval usage, int minSize) {
    int useageSize = usage.getSize(getInnerLength());
    int useageSizeWithMin = useageSize;
    boolean reallocationNeeded = false;
    if(useageSize < minSize) {
      useageSizeWithMin = minSize;
    }
    if (elements.length > useageSizeWithMin * maxOverhead)
      reallocationNeeded = true;
    else if (useageSizeWithMin > elements.length)
      reallocationNeeded = true;
    if(reallocationNeeded) {
      reallocate(usage, useageSizeWithMin);
      return useageSize == 0 ? new EmptyInterval() : new NonEmptyInterval(0, useageSize - 1);
    } else
      return usage;
  }
